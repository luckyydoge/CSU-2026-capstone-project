# service/stage_upload_service.py
import os
import hashlib
import importlib
import logging
...
STAGED_CODE_DIR = CONFIG.STAGED_CODE_DIR
...
class StageUploadService:
    ...
    @staticmethod
    def ensure_staged_code_dir():
        """确保staged_code目录存在"""
        os.makedirs(STAGED_CODE_DIR, exist_ok=True)
    ...
    def check_file_conflicts(filename: str, file_hash: str) -> Tuple[bool, str]:
        """检查文件冲突"""
        file_path = os.path.join(STAGED_CODE_DIR, filename)
    ...
    def validate_file(filename: str, content: bytes) -> Tuple[bool, str]:
        ...
        # 验证Python代码语法
        is_valid, validation_msg = StageUploadService.validate_python_code(content)
        if not is_valid:
    ...
    def upload_file(filename: str, content: bytes) -> Dict:
        ...
        Returns:
            包含上传结果的字典
        """
        StageUploadService.ensure_staged_code_dir()
        
        # 验证文件
        is_valid, validation_msg = StageUploadService.validate_file(filename, content)
        if not is_valid:
            raise ValueError(validation_msg)
        
        # 计算文件哈希
        file_hash = StageUploadService.calculate_file_hash(content)
        
        # 安全处理文件名
        safe_filename = StageUploadService.sanitize_filename(filename)
        
        # 检查文件冲突
        has_conflict, conflict_msg = StageUploadService.check_file_conflicts(safe_filename, file_hash)
        ...
        if has_conflict:
            if "same content" in conflict_msg:
                ...
                # 文件名冲突但内容不同，生成唯一文件名
                safe_filename = StageUploadService.generate_unique_filename(safe_filename, file_hash)
    ...
    def list_uploaded_files() -> Dict:
        ...
        files = []
        for filename in os.listdir(STAGED_CODE_DIR):
